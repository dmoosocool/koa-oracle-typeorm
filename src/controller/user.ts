...
export default class UserController {
  ...
  public static async getUser(ctx: Context): Promise<void> {
    ...
    if (user) {
      ...
      ctx.status = 400
      ctx.body = 'The user you are trying to retrieve does not exist in the db'
    }
  }
  ...
  public static async updateUser(ctx: Context): Promise<void> {
    ...
    userToBeUpdated.name = (ctx.request.body as unknown as User).name
    userToBeUpdated.email = (ctx.request.body as unknown as User).email
    ...
    if (errors.length > 0) {
      ...
      // return a BAD REQUEST status code and error message
      ctx.status = 400
      ctx.body = 'The user you are trying to update does not exist in the db'
    } else if (
  ...
  public static async deleteUser(ctx: Context): Promise<void> {
    ...
    if (!userToRemove) {
      // return a BAD REQUEST status code and error message
      ctx.status = 400
      ctx.body = 'The user you are trying to delete does not exist in the db'
